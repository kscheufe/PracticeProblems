...
class Solution {
    public String simplifyPath(String path) {
        ...
        {
            //adding path component to stack, checks for .., ., etc.
            if (path.charAt(i) == '/')
            {
                System.out.println("currentChar: " + path.charAt(i) + ", current string:" + current);
                if (current.equals(".."))//handles .. instances
                {
                    if (!components.isEmpty())
                        components.pop();
                }
                else if (current.equals("."))//handles /./ instances
                    ;
                else if (current == "")//handles // (empty) components
                    continue;
                else
        ...
        /*shouldn't need anymore now that i added a / to the end of the path
        if (current != "")//only happens if path doesn't end with /
        {
            components.push(current);//assuming it can't end with .. or .
        } */
        while (!components.isEmpty())
        {
            output = "/" + components.pop() + output;
        }
