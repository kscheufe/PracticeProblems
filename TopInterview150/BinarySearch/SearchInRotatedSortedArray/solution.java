...
class Solution {
    public int search(int[] nums, int target) {
        ...
        if (nums.length == 1) return -1;//basically an else
        ...
        while (l <= r) {
            ...
            if (nums[l] <= nums[index]) {//left half sorted
                if (nums[l] <= target && target < nums[index])//want to look left
                {
                    r = index-1;
                }
                else
                ...
                if (nums[index] < target && target <= nums[r])//want to look left
                {
                    l = index + 1;
                }
                else
